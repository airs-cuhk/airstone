...
float LaneDetector::display_img(Mat _frame, int _delay, bool _view) {        
    ...
    // Check if any VideoWriter did not open successfully
    if (!videoWriter1.isOpened() || !videoWriter2.isOpened() || !videoWriter3.isOpened()) {
        cerr << "Could not open one or more video writers!" << endl;
        return -1; // Return with error code
    }
    sliding_frame = detect_lines_sliding_window(gray_frame, _view);
    calc_curv_rad_and_center_dist();
    // Process the frame and save to video files if _view is true
    if (_view) {
        ...
        if(!new_frame.empty()) {
            videoWriter1.write(new_frame); // Write the frame to the first video
        }
        if(!sliding_frame.empty()) {
            videoWriter2.write(sliding_frame); // Write the frame to the second video
        }
        if(!resized_frame.empty()) {
            videoWriter3.write(resized_frame); // Write the frame to the third video
        }
        waitKey(3);
    }
    // Your existing code...
