...
void LocalRC::init(){
	...
	int OcrPubQueueSize;
	std::string speedTopicName;//added
	int speedQueueSize;
	std::string AckPubTopicName;//added
	int AckPubQueueSize;
	...
	nodeHandle_.param("subscribers/vehicle_status/topic", speedTopicName, std::string("/carla/ego_vehicle/vehicle_status")); //added
	...
	nodeHandle_.param("publishers/ackermann_command/topic", AckPubTopicName, std::string("/carla/ego_vehicle/ackermann_cmd")); //added
	nodeHandle_.param("publishers/ackermann_command/queue_size", AckPubQueueSize, 1);
	...
	AckPublisher_ = nodeHandle_.advertise<ackermann_msgs::AckermannDrive>(AckPubTopicName, AckPubQueueSize);
...
//	SatVel_ = msg.u_k;	//saturated velocity
	...
	// Random number generator
        ...
        std::mt19937 gen(rd());
        std::uniform_int_distribution<> dis(1, 2); // 50% chance of setting velocity to 0

        double receivedVel = msg.velocity;
        //if (dis(gen) == 1) {  // 50% chance to set the velocity to 0
            //receivedVel = 0;  // Set velocity to 0
        //}
        receivedVel = 0;
        CurVel_ = receivedVel;
}*/
...
float LocalRC::ChangeDegree(float AngleDegree){ //added
    AngleDegree /= 90.00;
    if (AngleDegree > 1.22){
    	return 1.22;
    }
    else if (AngleDegree < -1.22){
    	return -1.22;
    }
    else{
    	return AngleDegree;
...
void LocalRC::AckPub(){ //added
    ChangedDegree_ = ChangeDegree(AngleDegree_);
    ackermann_msgs::AckermannDrive msg;
    //msg.header.stamp = ros::Time::now();
    // Set message values
    msg.steering_angle = ChangedDegree_;
    msg.steering_angle_velocity = 0.4; //set
    msg.speed = TarVel_;
    msg.acceleration = 0.0;
    msg.jerk = 0.0;

    // Publish the message
    AckPublisher_.publish(msg);
}
