...
bool ScaleTruckController::readParameters() {
  ...
  nodeHandle_.param("image_view/wait_key_delay", waitKeyDelay_, 3);//changed
...
void ScaleTruckController::init() {
  ...
  nodeHandle_.param("subscribers/camera_reading/queue_size", imageQueueSize, 100);
...
/*******/  
  static std::vector<double> processing_times; // Store processing times for each frame
...
/*******/
    // End timing after processing is done, right before any display or saving of the frame
    end_time = static_cast<double>(cv::getTickCount());
    // Calculate the processing time in seconds for this frame
    double processing_time = (end_time - start_time) / cv::getTickFrequency();
    processing_times.push_back(processing_time); // Save the processing time
    
    // After processing all frames, you can calculate the average
    if (processing_times.size() > 0) {
        double sum = std::accumulate(processing_times.begin(), processing_times.end(), 0.0);
        double average_time = sum / processing_times.size();

        // You could print out the average time here or return it if you change the function signature
        std::cout << "Average lane detection time per frame: " << average_time << " seconds." << std::endl;
    }
/*******/
}
...
void ScaleTruckController::displayConsole() {
  ...
  printf("\nCurrent Cycle Time      : %3.3f ms", OneCycleTime_);
  printf("\nAverage Cycle Time      : %3.3f ms", CycleTime_);
  printf("\n");
}
...
void ScaleTruckController::spin() {
  ...
  while(!controlDone_ && ros::ok()) {
    ...
    OneCycleTime_ = ((end_time.tv_sec - start_time.tv_sec) * 1000.0) + ((end_time.tv_usec - start_time.tv_usec) / 1000.0);
    cnt++;
